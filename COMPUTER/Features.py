...
#from Automations import wolframalpha
...
def GoogleSearch(term):
    ...
    oooooo = open('D:\\COMPUTER\\Data.txt','a')
    ...
    os.startfile('D:\\COMPUTER\\DataBase\\ExtraPro\\start.py')
...
def Alarm(query):
    ...
    TimeHere=  open('D:\\COMPUTER\\Data.txt','a')
    TimeHere.write(query)
    TimeHere.close()
    os.startfile("D:\\COMPUTER\\DataBase\\ExtraPro\\Alarm.py")
...
def DownloadYouTube():
    ...
    def Download(link):
        ...
        video.download('D:\\ computer \\VIDEOS\\')
    ...
    os.startfile('D:\\COMPUTER\\VIDEOS\\')

def SpeedTest():

    os.startfile("D:\\COMPUTER\\DataBase\\Gui Programs\\SpeedTestGui.py")
...
def My_Location():
    ...
    Speak(f"Sir , You Are Now In { country, state} .")
...
def Temp(query):
    ...
    Term =Term.replace("computer","")
    Term =Term.replace("in","")
    Term =Term.replace("what is the ","")
    Term =Term.replace("temprature","")

    temp_query= str(Term) 

    #if'outside' in temp_query:
   #     var1 = "Temprature in kanyakumari"
#
